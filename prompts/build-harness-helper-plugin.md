...
- 점검 워크플로우·적용 가능 항목: `references/harness-checker.md` — **0-2장 적용 가능성 재판정 규칙(✅/△/❌)**,
  6단계 절차, 비용/성능/보안 체크리스트(2~4장, ✅/△ 25항목 — 비용 8·성능 6·보안 11), 6장 레시피, 5장 기록부,
  7장 구조적 불가(❌), 8장 로드맵, 0-3장 harness.conf 설정 외부화 패턴
...
1. 소스 인벤토리: `AGENTS.md`·`references/` 전 항목을 읽어 (a) 멤버 5인 → 에이전트 매핑, (b) harness-checker
   점검 절차 → check skill Phase(점검·기록까지) + 보완 구현은 apply skill로 분리, (c) **적용 가능 항목
   집합(✅/△)**을 확정. corp-research 파일별 규격을 대조 기준으로 사용.
...
4. 스킬 생성 `skills/check/SKILL.md` — harness-checker 점검 절차를 5단계 Phase로 구현(보완 구현은 apply로 분리):
   - frontmatter: `name:"check"`·`description`(점검·재점검·갱신·로드맵 키워드 포함)·`triggers`·
   ...
   - Phase 0 입력(대상 플러그인 경로 확인) → Phase 1 구성요소 인벤토리 + **⓪ 적용 가능성 재판정**(클로니) →
     ...
     Phase 3 기록부·로드맵 통합(클로니, 미적용 항목을 영역별로 그룹화해 기록 — 보완 선택·구현은 하지 않음) →
     Phase 4 완료 보고(미적용 보완은 `apply` 스킬로 안내)
   ...
   - 추가 스킬 `skills/apply/SKILL.md`: 점검 기록의 **미적용 항목 정리(영역→리스크→ID 문단 포맷)**를 본문 출력하고
     사용자가 **항목 ID(C1·P2·S3 등)를 입력**해 선택(AskUserQuestion 미사용) → 빌디로 구현(한국어 산출)·검증 후
     기록 갱신(취소선+`[적용됨]`으로 ID 보존). 기록 미존재 시 check 선호출.
     **check와 분리된 독립 스킬**로 사용자가 언제든 단독 실행함(check는 apply를 호출하지 않음)
5. 커맨드 생성 `commands/check.md`·`commands/apply.md`: 각 frontmatter(`description`·
...
├── skills/
│   ├── check/SKILL.md         # 5단계 점검 워크플로우 (✅/△ 항목만)
...
- 완료조건(객관적 증거):
  ...
  - `agents/` 4파일·`skills/check/SKILL.md`·`commands/check.md`가 실제 생성됨
...
(3) `commands/check.md` — 본문 1줄 (corp-research commands/research.md 규격 준용)

```markdown
---
description: 대상 Claude Code 플러그인의 비용·성능·보안 하네스 통제 중 적용 가능 항목을 5단계로 점검·로드맵 도출

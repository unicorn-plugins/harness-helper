...
4. 스킬 생성 `skills/check/SKILL.md` — harness-checker 6단계를 Phase로 구현:
   - frontmatter: `name:"check"`·`description`(점검·보완·재점검·갱신 키워드 포함)·`triggers`·
   ...
   - Phase 0 입력(대상 플러그인 경로 확인) → Phase 1 구성요소 인벤토리 + **⓪ 적용 가능성 재판정**(클로니) →
     ...
     Phase 4 선택 항목 구현(apply 스킬로 위임) → Phase 5 완료 보고
   ...
   - 추가 스킬 `skills/apply/SKILL.md`: 점검 기록의 미적용(✅/△) 항목을 **영역별(비용·성능·보안) 질문**으로
     상세 항목 복수 선택(AskUserQuestion 영역별)하여 빌디로 구현(한국어 산출)·검증 후 기록 갱신. 기록 미존재 시 check 선호출.
     독립 실행 가능하며 **check Phase 4가 사용자 선택 시 apply를 호출**하도록 연결
...
├── skills/
│   ├── check/SKILL.md         # 6단계 점검 워크플로우 (✅/△ 항목만)

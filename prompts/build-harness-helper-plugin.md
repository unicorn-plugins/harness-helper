...
4. 스킬 생성 `skills/harness-check/SKILL.md` — harness-checker 6단계를 Phase로 구현:
   ...
   - Phase 0 입력(대상 플러그인 경로 확인) → Phase 1 구성요소 인벤토리 + **⓪ 적용 가능성 재판정**(클로니) →
     ...
     Phase 3 기록부·로드맵 통합(클로니, AskUserQuestion `multiSelect`로 **적용 가능 보완 항목 복수 선택**) →
   ...
   - 추가 스킬 `skills/harness-apply/SKILL.md`: 점검 기록의 미적용(✅/△) 항목을 사용자가 **복수 선택**
     (AskUserQuestion `multiSelect`)하여 빌디로 구현·검증 후 기록 갱신. 점검 기록 미존재 시 harness-check 선호출.

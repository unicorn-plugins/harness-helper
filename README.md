...
- **단일 명령 6단계 점검**: `/harness-helper:check {대상경로}` 한 번으로 인벤토리→재판정→병렬 점검→
  기록·로드맵→보완 구현→보고 수행
...
- **선택 보완 구현(언제든)**: `apply`로 미적용 항목을 **복수 선택**해 빌디가 6장 레시피로 구현·검증 후  
...
| 유형 | 이름 | 네임스페이스 호출 | 역할 |
|------|------|------------------|------|
| 스킬 | `check` | `/harness-helper:check` | 오케스트레이션·6단계 점검 워크플로우 |

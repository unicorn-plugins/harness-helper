...
- **단일 명령 6단계 점검**: `/harness-helper:harness-check {대상경로}` 한 번으로 인벤토리→재판정→병렬 점검→
  기록·로드맵→보완 구현→보고 수행
...
- **선택 보완 구현(언제든)**: `harness-apply`로 미적용 항목을 **복수 선택**해 빌디가 6장 레시피로 구현·검증 후  
  점검 기록을 갱신. 점검 직후뿐 아니라 추후 세션에서도 독립 실행 가능
...
| 유형 | 이름 | 네임스페이스 호출 | 역할 |
|------|------|------------------|------|
| 스킬 | `harness-check` | `/harness-helper:harness-check` | 오케스트레이션·6단계 점검 워크플로우 |
| 스킬 | `harness-apply` | `/harness-helper:harness-apply` | 미적용(✅/△) 항목 복수 선택·보완 구현 |
...
```text
# 점검 + (선택 시) 보완 구현 위임
/harness-helper:harness-check C:\path\to\target-plugin

# 미적용 항목을 언제든 복수 선택하여 보완 구현
/harness-helper:harness-apply C:\path\to\target-plugin
```
...
├── skills/
│   ├── harness-check/SKILL.md         # 6단계 점검 워크플로우

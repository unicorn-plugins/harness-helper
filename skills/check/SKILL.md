...
대상 Claude Code 플러그인의 비용·성능·보안 하네스 통제 중 **적용 가능(✅/△) 항목**을 5단계로 점검하고,  
미적용 항목의 보완방안·적용 로드맵을 도출함. 근거 문서: `${CLAUDE_PLUGIN_ROOT}/references/harness-checker.md`.  
보완 구현은 점검 범위에서 분리됨 — 미적용 항목 구현은 `apply` 스킬(`/harness-helper:apply`)로 별도 진행함.

## 팀 역할 분담

| 역할 | 닉네임 | 담당 Phase | subagent_type |
|------|--------|-----------|---------------|
| 오케스트레이터 | 클로니 | Phase 0·1·3·4 | 메인 (직접 수행) |
...
> 미적용 항목 보완 구현(빌디 `implementation-engineer`)은 이 스킬에서 분리됨 — `apply` 스킬이 전담함.
...
## Phase 3. 기록부·로드맵 통합 `[클로니 — 직접 수행]`

1. 세 결과를 harness-checker 5장 **영역별 요약표**(영역·대상·Y·N·적용률·미적용 Critical/High)로 통합
2. **미적용 항목 정리는 표가 아닌 문단(계층) 형식**으로 작성함 (harness-checker 5장 형식):
   - 계층: ① 영역(비용·성능·보안) → ② 리스크 `리스크명(약어)`(돌·토·폭/멈·느·할/침·유·권) →  
     ③ 항목 `{ID}. [등급] 항목명` + `- 현재 문제:`(1~2줄) + `- 개선 방안:`(1~2줄)
   - ID: 영역 접두사(비용 `C`·성능 `P`·보안 `S`) + 영역별 일련번호(C1·C2·P1·S1…)
   - 정렬: 영역(비용→성능→보안) → 리스크(약어 표준순) → 항목(등급 Critical→High→Medium→Low, 동급 항목명순);  
     ID는 이 순서대로 영역별 1부터 부여
   - 재점검 시: **기존 ID 보존**(취소선 ID 재사용 금지), 적용된 항목은 `~~취소선~~ [적용됨]`으로 잔류,  
     신규 미적용만 해당 영역 최대번호+1부터 증분 채번 (상세 규칙: harness-checker 5장)
3. `{대상경로}/harness/harness-adoption-plan.md`에 요약표·미적용 항목 정리(문단)·8장 로드맵 작성 (corp-research 형식 준용)
4. 보완 구현은 `apply` 스킬이 이 미적용 항목 정리(ID 목록)를 기준으로 수행함 (이 스킬은 항목을 선택·구현하지 않음)

## Phase 4. 완료 보고 `[클로니 — 직접 수행]`
...
■ 다음 단계   : 미적용 항목 보완은 /harness-helper:apply {대상경로} 로 별도 진행
```
...
> 보완 적용 결과(변경 파일)는 이 스킬의 산출물이 아님 — `apply` 스킬이 구현·기록함.

...
- harness-adoption-plan.md에서 **미적용(N)·적용 가능(✅/△)** 항목을 추출
- **영역별(비용/성능/보안)로 그룹화**하여 후보 목록 구성. 각 영역 내 항목은 심각도순(Critical→High→Medium→Low)  
  정렬, 항목마다 상세 보완 내용(6장 레시피 요약) 병기
- 미적용 항목이 있는 영역만 선택 단계 대상으로 남김. 전 영역에 미적용 항목이 없으면 → "보완할 미적용 항목 없음"  
  보고 후 종료

## Phase 2. 영역별 상세 항목 복수 선택 `[클로니 — 직접 수행]`

- AskUserQuestion을 **영역별 질문**으로 구성 (미적용 항목이 있는 영역만, 최대 3개 질문: 비용·성능·보안)
  - 각 질문 `multiSelect: true`, 옵션 = 해당 영역의 미적용 상세 항목(심각도·보완 요약 표기)
  - 사용자가 영역마다 구현할 상세 항목을 **복수 선택**
  - 한 영역 미적용 항목이 4개 초과 시: 심각도 높은 순 4개씩 제시하고 나머지는 다음 라운드로 반복 질문(옵션 4개 한도)
- 세 영역의 선택을 합쳐 `{선택 항목 목록}` 구성. 선택 0건이면 종료, 1건 이상이면 Phase 3
...
- 영역별 질문(비용·성능·보안, 각 `multiSelect: true`)으로 사용자가 선택한 상세 항목만 구현

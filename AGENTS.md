...
- 역할: 플러그인의 비용 하네스 통제 점검(harness-checker 2장) — 최대 반복 횟수·반복 패턴 감지·입력 절삭·
  요청별 사용량 측정·호출 수 제한·Budget 한도·핸드오프 요약·부분 결과 반환 항목의 ⓪ 적용가능성(✅/△/❌)
  재판정·△→✅ 승격, ② Y/N 판정, ③ 근거(파일:라인) 기재 및 6장 레시피 기반 보완방안 도출.
...
| 2026-06-21 | harness-checker 개정 정합성 반영 — 점검가 대상 항목 동기화(비용 12→8·성능 9→6)·harness.conf 수집·자가점검 라벨 | agent·AGENTS·prompt·README | 체크리스트 재설계(32→25) 정합 |
| 2026-06-21 | 하네스 설정 외부화 포맷 harness.conf 확정 (v1.0.5) — 주석 지원 KEY=value(dotenv)·의존성 없는 Node 로더(hooks/lib/harness-config.mjs)로 §0-3 재작성·전파 | reference·skill·agent·prompt·plugin | yq/jq 불가(Windows·node)+주석 필수 반영 |
